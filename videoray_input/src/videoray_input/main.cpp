...
int main(int argc, char **argv)
{
     ...
     ros::param::param<std::string>("port", joystick_dev, "/dev/input/js0");     
     std::cout << "Joystick Device Port: " << joystick_dev << std::endl;
     ...
     ros::Publisher joystick_pub_ = n_.advertise<sensor_msgs::Joy>("/videoray_input/joystick", 1);     

     // Create a joystick instance
     Joystick joy_;
     if (joy_.init(joystick_dev) == -1) {
          ROS_ERROR_STREAM("Error connecting joystick port, exit...");
          ros::shutdown();
     }
