#ifndef TRITECH_MICRON_MESSAGETYPES_H
#define TRITECH_MICRON_MESSAGETYPES_H
...
struct mtHeadCommandMsg {
  mtHeadCommandMsg(uint16_t _nBins = 200,
                   float _range = 10,
                   float _VOS = 1500,
                   uint8_t _angleStep = 32,
                   uint16_t _leftLimit = 1,
                   uint16_t _rightLimit = 6399,
                   bool _cont = true) :
      nBins(_nBins),
      range(_range),
      VOS(_VOS),
  ...
  std::vector<uint8_t> construct() {
    ...
    std::vector<uint8_t> msg = {
        0x00, 0x40, 0x30, 0x30, 0x34, 0x43, 0x4C, 0x00, 0xFF, 0x02, 0x47, 0x13, 0x80, 0x02, 0x1D, 0x01, 0x23, 0x02,
...
struct mtHeadDataMsg {
  ...
  mtHeadDataMsg(Message const &msg) {
    ...
    switch (msg.data[18]) {
      case 0:sweepCode = Scanning_Normal;
        break;
      case 1:sweepCode = Scan_AtLeftLimit;
        break;
      case 2:sweepCode = Scan_AtRightLimit;
        break;
      case 5:sweepCode = Scan_AtCentre;
        break;
      default:
        std::cerr << __FUNCTION__ << ":" << __LINE__ <<
    ...
    switch (rangeTp) {
      case 0:rangeUnits = meters;
        break;
      case 1:rangeUnits = feet;
        break;
      case 2:rangeUnits = fathoms;
        break;
      case 3:rangeUnits = yards;
        break;
      default:
        std::cerr << __FUNCTION__ << ":" << __LINE__ <<
...
#endif // TRITECH_MICRON_MESSAGETYPES_H
