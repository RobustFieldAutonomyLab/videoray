...
struct mtHeadCommandMsg {
    mtHeadCommandMsg(uint16_t _nBins = 200, float _range = 10, float _VOS = 1500, uint8_t _angleStep=32, uint16_t _leftLimit=1, uint16_t _rightLimit=6399, bool _cont=true) :
        nBins(_nBins), range(_range), VOS(_VOS), angleStep(_angleStep),leftLimit(_leftLimit),rightLimit(_rightLimit),cont(_cont) {
    }
...
struct mtHeadDataMsg {
    ...
    mtHeadDataMsg(Message const& msg) {
        ...
        case 0:
            ...
            break;
        case 1:
            sweepCode = Scan_AtLeftLimit;
            break;
        case 2:
            sweepCode = Scan_AtRightLimit;
            break;
        case 5:
            sweepCode = Scan_AtCentre;
            break;
        default:
            std::cerr << __FUNCTION__ << ":" << __LINE__ <<
