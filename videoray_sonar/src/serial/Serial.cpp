...
void Serial::connect()
{
        ...
        write(cmd,1);
                                ...
                                printf("INFO: %s : %s\n", entryName.c_str(), deviceString.c_str());
                ...
                if(itsCmdDevName == "")
                {
                        printf("FATAL ERROR: Could Not Find Serial Device Matching Descriptor (%s)\n", itsDeviceDescription.c_str());
                }
        } else {
...
void Serial::configureSearch(const std::string DeviceDescription, const int speed, const std::string SearchPrefix, const char *format,
                ...
                const int tout)
{
        printf("INFO: Search for %s in speed %d",DeviceDescription.c_str(),speed);
        itsDevName = "search";
                ...
                const int tout)
{
  itsDevName = dev;
        ...
        if (strlen(format) != 3) printf("FATAL ERROR: Incorrect format string: %s", format);
        ...
        switch(format[0]) {
                ...
                default: printf("FATAL ERROR: Invalid charbits: %c (should be 5..8)", format[0]);
        }
        ...
        switch(format[1]) {
                ...
                default: printf("FATAL ERROR: Invalid parity: %c (should be N,E,O)", format[1]);
        }

        switch(format[2]) {
                case '1': itsStopBits = 1; break;
                case '2': itsStopBits = 2; break;
                default: printf("FATAL ERROR: Invalid stopbits: %c (should be 1..2)", format[2]);
        }
}
...
serialError Serial::setBlocking(const bool blocking)
{
        ...
        if (flags == -1) printf("ERROR: Cannot get flags\n");
        if (blocking) flags &= (~O_NONBLOCK); else flags |= O_NONBLOCK;
        if (fcntl(dev, F_SETFL, flags) == -1) printf("ERROR: Cannot set flags\n");
...
void Serial::perror(void)
{
        switch(serialErrno)
        {
                case serialErrSuccess:
                        printf("ERROR: Serial::OK\n");
                        return;
                case serialErrOpenNoTty:
                        printf("ERROR: Serial::Open() No TTY Failed\n");
                        break;
                case serialErrOpenFailed:
                        printf("ERROR: Serial::Open() Failed\n");
                        break;
                case serialErrSpeedInvalid:
                        printf("ERROR: Serial::setSpeed() Invalid Speed Param\n");
                        break;
                case serialErrFlowInvalid:
                        printf("ERROR: Serial::setFlow() Invalid Flow Param\n");
                        break;
                case serialErrParityInvalid:
                        printf("ERROR: Serial::setParity() Invalid parity Param\n");
                        break;
                case serialErrCharsizeInvalid:
                        printf("ERROR: Serial::setCharSize() Invalid Char Size\n");
                        break;
                case serialErrStopbitsInvalid:
                        printf("ERROR: Serial::setStopBits() Invalid Stop Bits\n");
                        break;
                case serialErrOptionInvalid:
                        printf("ERROR: Serial::read() invalid option\n");
                        break;
                case serialErrOutput:
                        printf("ERROR: Serial::read() err output\n");
                        break;
                case serialErrReadFailed:
                        printf("ERROR: Serial::read() Failed\n");
                        break;
                case serialErrReadTimedOut:
                        printf("ERROR: Serial::read() Timed Out\n");
                        break;
                case serialErrWriteFailed:
                        printf("ERROR: Serial::Write() Failed\n");
                        break;
                case serialErrFcntlFailed:
                        printf("ERROR: Serial::Fcntl() Failed\n");
                        break;
                case serialErrTcGetAttrFailed:
                        printf("ERROR: Serial::tcgetattr() Failed\n");
                        break;
